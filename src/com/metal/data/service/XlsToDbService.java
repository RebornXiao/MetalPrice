...
public class XlsToDbService {
	...
	public static List<AgData> getAllByDb() {
		...
		try {
			...
			while (rs.next()) {
				...
				Integer min_price = rs.getInt("min_price");
	 ...
	 * @return
	 */
	public static List<AgData> getAll5MinByDb() {
		List<AgData> list = new ArrayList<AgData>();
		try {
			DBHelper db = new DBHelper();
			String sql = "select * from t_ag_data_5min";
			ResultSet rs = db.Search(sql, null);
			while (rs.next()) {
				int id = rs.getInt("id");
				String time = rs.getString("time");
	 ...
	 * @return
	 */
	public static List<AgData> getAll1hByDb() {
		List<AgData> list = new ArrayList<AgData>();
		try {
			DBHelper db = new DBHelper();
			String sql = "select * from t_ag_data_1h";
			ResultSet rs = db.Search(sql, null);
			while (rs.next()) {
				int id = rs.getInt("id");
				String time = rs.getString("time");
	 ...
	 * @return
	 */
	public static List<AgData> getAll5MinByExcel(String file) {
		List<AgData> list = new ArrayList<AgData>();
		try {
			Workbook rwb = Workbook.getWorkbook(new File(file));
			Sheet rs = rwb.getSheet(0);// 或者rwb.getSheet(0)
			...
			for (int i = 1; i < rows; i++) {
				for (int j = 0; j < clos; j++) {
					...
					String date = rs.getCell(j++, i).getContents();// 日期//默认最左边编号也算一列
																	// 所以这里得j++
					String name = "ag";// rs.getCell(j++, i).getContents();//
										// 合约名称
					String open = rs.getCell(j++, i).getContents();// 开盘价
					String max = rs.getCell(j++, i).getContents();// 最高价
					String min = rs.getCell(j++, i).getContents();// 最低价
					String close = rs.getCell(j++, i).getContents();// 收盘价
//					String change = "0";
//					String change_range = "0";
//					if (i != 1) {
					String	changeNum = rs.getCell(j++, i).getContents();// 涨跌额
//						change_range = rs.getCell(j++, i).getContents();// 涨跌幅度
//					}
//					String average = rs.getCell(j++, i).getContents();// 加权平均价
//					j++;
					String turnover_money = rs.getCell(j++, i).getContents();// 成交金额
					...
					String change = new String(changeNum);
					if (change.startsWith("-")) {
						change = change.replace("-", "");
					}
					double changeDou = Double.parseDouble(change);
...
//					}
	 ...
	 * @return
	 */
	public static List<AgData> getAll1hByExcel(String file) {
		List<AgData> list = new ArrayList<AgData>();
		try {
			Workbook rwb = Workbook.getWorkbook(new File(file));
			Sheet rs = rwb.getSheet(0);// 或者rwb.getSheet(0)
